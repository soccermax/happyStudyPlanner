...
const triggerNotification = async (proactiveEvent, accessToken, live = false) => {
    if (!accessToken) {
        throw new Error("Can't find accessToken");
    }
    const url = `https://api.eu.amazonalexa.com${live ? '/v1/proactiveEvents' : '/v1/proactiveEvents/stages/development'}`;
    const config = {
        data: getNotificationTemplate(),
        url,
        method: "POST",
        headers: {
    ...
    return {
        ...
        event: {
            ...
            payload: {
                ...
                messageGroup: {
                    creator: {
                        name: "einem unbearbeiteten Learning Agreement von Christian Axtmann"
                    },
                    count: 1
                },
            },
